...
function TreeNode({
  ...
  if (node.type === "directory") {
    return (
      <div className={`sidebar-node mb-[2px] border-transparent ${containerClassName}`} {...dragHandlers}>
        <button
          className="sidebar-directory flex items-center gap-2 w-full text-left rounded-md mx-2 hover:bg-sidebar-accent/50 transition-colors"
          style={{ paddingLeft: `${depth * 14 + 10}px`, paddingRight: "8px", paddingTop: "6px", paddingBottom: "6px" }}
          onClick={() => {
            if (depth === 0) {
  ...
  return (
    <div className={`group relative flex items-center mb-[2px] border-transparent ${containerClassName}`} {...dragHandlers}>
      <div
        className={`sidebar-file flex items-center rounded-md mx-2 min-w-0 ${activePath === node.path ? "is-active bg-sidebar-accent text-sidebar-accent-foreground" : "hover:bg-sidebar-accent/50"}`}
        style={{ paddingLeft: `${depth * 14 + 10}px`, paddingRight: "32px", paddingTop: "6px", paddingBottom: "6px", width: "calc(100% - 16px)" }}
      >
        ...
        {isRenaming ? (
          ...
          <button className="flex-1 text-left truncate text-sm min-w-0 overflow-hidden" onClick={() => onOpenFile(node.path)} type="button">
            {node.name}
          </button>
        )}
      </div>
      {!isRenaming && (
        <div className="absolute right-4 opacity-0 group-hover:opacity-100 transition-opacity flex items-center flex-shrink-0 pointer-events-none group-hover:pointer-events-auto">
            ...
            {showMenu && (
              <>
                <button
                  ...
                  type="button"
                />
                <div className="absolute right-0 top-full mt-1 bg-popover border border-border rounded-md shadow-lg py-1 z-50 min-w-[140px]">
                  <button
    ...
    <aside className="sidebar flex flex-col h-full w-[280px] bg-sidebar border-r border-border">
      <div className="flex items-center px-4 h-14 border-b border-border flex-shrink-0">
        <div className="flex items-center gap-2 text-foreground font-semibold">
          <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-primary"><path d="M12 20h9" /><path d="M16.5 3.5a2.12 2.12 0 0 1 3 3L7 19l-4 1 1-4Z" /></svg>
          Typist
        </div>
      </div>
